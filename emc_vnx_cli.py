...
class CommandLineHelper(object):
    ...
    CLI_RESP_PATTERN_LUN_USED_BY_FEATURE = (
        ...
        if rc != 0 or out.strip():
            ...
            if rc == 9 and self.CLI_RESP_PATTERN_LUN_NOT_EXIST in out:
                LOG.warning(_LW("LUN is already deleted, LUN name %(name)s. "
                    ...
                    self.CLI_RESP_PATTERN_LUN_USED_BY_FEATURE in out):
                ...
                raise EMCLunUsedByFeature(cmd=command_delete_lun,
                                          rc=rc,
                                          out=out)
            elif (self.CLI_RESP_PATTERN_LUN_IN_SG_1 in out or
                    self.CLI_RESP_PATTERN_LUN_IN_SG_2 in out):
    ...
    def create_cgsnapshot(self, cgsnapshot, keep_for=None):
        ...
        :param cgsnapshot: cg snapshot object
        :param keep_for: the specified time after
                         which the snapshot will expire
        """
    ...
    def create_snapshot(self, lun_id, name, keep_for=None):
        """Create a snapshot for LUN.

        :param lun_id: VNX LUN ID
    ...
    def _filter_iscsi_ports(self, all_ports, io_ports):
        ...
        if len(io_ports) != len(valid_ports):
            ...
            for invalid in invalid_port_set:
                LOG.warn(_LW('Invalid iSCSI port %(sp)s-%(port)s-%(vlan)s '
...
class EMCVnxCliBase(object):
    ...
    def _parse_ports(self, io_port_list, protocol):
        ...
        if len(invalid_ports) > 0:
            msg = _LE('Invalid %(protocol)s ports %(port)s specified '
    ...
    def create_volume_from_snapshot(self, volume, snapshot):
        ...
        1. Copy a new snap from source snap and allow RW for it.
           (if snapcopy=True).
        2. Create a snap mount point (SMP) for the snapshot.
        3. Attach the snapshot to the SMP created in the first step.
        4. Create a temporary lun prepare for migration.
           (if snapcopy=False).
        5. Start a migration between the SMP and the temp lun
           (if snapcopy=False).
        """
        self._volume_creation_check(volume)
    ...
    def create_cloned_volume(self, volume, src_vref):
        ...
        if consistencygroup_id:
            tmp_cgsnapshot_name = self._construct_snap_name(volume)
    ...
    def manage_existing(self, volume, ref):
        ...
        model_update = {'provider_location':
                        self._build_provider_location(ref['id'])}
...
class EMCVnxCliArray(EMCVnxCliBase):
    ...
    def get_target_storagepool(self, volume, source_volume=None):
        ...
        if (provider_location and
                self._extract_provider_location(provider_location,
                                                'version')):
            return pool
        else:
...
class AttachSnapTask(task.Task):
    ...
    """
    def execute(self, client, volume, new_snap_name, *args, **kwargs):
...
class ModifySnapshotTask(task.Task):
    ...
    def execute(self, client, new_snap_name, temp_snap, *args, **kwargs):
    ...
    def execute(self, client, snapshot, source_lun_id,
        ...
        if snapshot['consistencygroup_id']:
            ...
            LOG.info(_LI('Create snapshot: %(snapshot)s: volume: %(volume)s'),
                     {'snapshot': snapshot_name,
                      'volume': volume_name})
            client.create_snapshot(source_lun_id,
                                   snapshot_name,
                                   keep_for)
